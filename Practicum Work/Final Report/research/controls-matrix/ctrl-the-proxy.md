...
- Palo Alto Unit 42, "'Shai-Hulud' Worm Compromises npm Ecosystem in Supply Chain Attack" — https://unit42.paloaltonetworks.com/npm-supply-chain-attack/ (accessed 2026-07-15) → `unit42-shai-hulud` *(already deferred in row 1; reused here, not a new key)*
- A. Freund, "backdoor in upstream xz/liblzma leading to ssh server compromise" (oss-security), CVE-2024-3094 — https://www.openwall.com/lists/oss-security/2024/03/29/4 (accessed 2026-07-15) → `openwall-xz-backdoor` *(already deferred in row 4; reused here, not a new key)*
...
|---|:--:|---|---|
| Stolen credential | `✓` | **Catches** the Shai-Hulud archetype — one stolen seat casts one vote of *m*, bound to the artifact; *m*−1 independent barriers still stand, so the token-harvest that republished under a legitimate identity finds no unilateral path here ([CORE-1](../../../../docs/threat-model/CORE-1-single-approver-account-compromise.md), executably demonstrated, bucket ①). **Misses** nothing at this cell — a colluding quorum is caveat 2, not column A. | `unit42-shai-hulud`, CORE-1 |
...
- None new. Reuses `unit42-shai-hulud` (Unit 42, "'Shai-Hulud' Worm Compromises npm Ecosystem," 2025; deferred in row 1) and `openwall-xz-backdoor` (A. Freund, oss-security disclosure of CVE-2024-3094, 2024-03-29; deferred in row 4). The proxy's documented behavior anchors to the repo's own specs (`constraints.md`, threat catalog), which are not bibliography entries.
