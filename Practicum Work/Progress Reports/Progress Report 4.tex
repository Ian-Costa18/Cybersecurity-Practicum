...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item As evidence that the design generalizes, the same approval core drives a second post-approval outcome (a service grant to a traditional web proxy forward-auth).
...
% Summary — owned threats by evaluation bucket
\begin{tabular}{@{}lll@{}}
  \toprule
  \textbf{Bucket} & \textbf{Count} & \textbf{Threats} \\
  \midrule
  % TODO: Table goes off of the page.
  ① Executably demonstrated & 11 & CORE-1, CORE-2, DOS-1, HOST-2, IDENT-1, IDENT-2, IDENT-5, PUB-1, PUB-2, VOTE-2, VOTE-3 \\
  ② Argued by design & 8 & CODE-1, CORE-4, CRYPTO-1, HOST-3, IDENT-4, INFO-1, VOTE-1, VOTE-4 \\
  ③ Operator-enforced & 5 & DOS-2, HOST-4, HOST-5, IDENT-6, VOTE-5 \\
  ④ Accepted limitation & 5 & CODE-2, CORE-3, DOS-3, DOS-4, HOST-1 \\
  \bottomrule
\end{tabular}

Net delta: 24 introduced · 5 improved · 4 unchanged (inherited).
