...
rows** — all three born enrolled (Mode-B) and revealed together by Act 0's single button.
...
# --- Every co-owner: readable public key, ciphertext private key -----------


def test_every_co_owner_public_key_is_readable_private_key_is_ciphertext(
    session: Session,
) -> None:
    # Act 0's single reveal shows all three co-owners at once, so the crypto beat must
    # hold for each: a readable public key beside a ciphertext private key, read from the
    # real row.
    demo_lib.provision_demo_team(session)

    for person in demo_lib.CO_OWNERS:
        assert person.provisioning == "mode-b"  # the whole cast is born enrolled now
        ...
        recovered = crypto.decrypt_private_key(
            ...
            crypto.derive_enc_key(person.password, state.key_salt),
...
def test_every_co_owner_totp_secret_is_ciphertext_bound_to_the_password(
    session: Session,
) -> None:
    demo_lib.provision_demo_team(session)
    ...
    for person in demo_lib.CO_OWNERS:
        ...
        # It decrypts under the right password...
        enc_key = crypto.derive_enc_key(person.password, state.totp_salt)
        crypto.decrypt_totp_secret(state.totp_secret_ciphertext, enc_key, crypto.totp_aad(user.id))
...
def test_mode_b_co_owners_are_born_enrolled_and_can_sign(session: Session) -> None:
    ...
    assert len(born_enrolled) == 3  # all three co-owners are born enrolled
