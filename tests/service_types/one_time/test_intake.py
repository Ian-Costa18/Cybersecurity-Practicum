...
from msig_proxy.approvals.snapshot import UnknownApproverError
...
def test_rejects_a_service_referencing_an_unknown_approver(session: Session) -> None:
    ...
    with pytest.raises(UnknownApproverError, match="nobody"):
        create_publish_request(
            ...
            requester=requester,
            service_name="pypi",
            service=_service(quorum=2, approvers=["alice", "nobody"]),
            package_name="foo",
            package_version="1.2.3",
            filename="foo-1.2.3.tar.gz",
            content=b"bytes",
...
def _create(session: Session, requester: User, approvers: list[str]) -> ApprovalRequest:
    return create_publish_request(
        ...
        requester=requester,
        service_name="pypi",
        service=_service(quorum=2, approvers=approvers),
        package_name="foo",
        package_version="1.2.3",
        filename="foo-1.2.3.tar.gz",
        content=b"bytes",
