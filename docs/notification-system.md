...
- **Admin notifications.** Notify admins of security-relevant events — request floods / approval-fatigue bursts (subscribing `request.created` for [threat model T12](threat-model/00-overview.md) anomaly detection), repeated `action.failed`, etc. No admin-facing notifications exist in MVP.
