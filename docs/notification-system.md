...
If email delivery fails — or email is disabled entirely via `notifications.email.enabled: false` — the proxy does **not** fail hard. For notifications that carry an actionable link (Approval Links, enrollment links), the link is displayed in the **authentication portal** so an operator can copy and distribute it manually (`fallback_to_portal: true`). This is the degraded-mode path, not the primary one.
...
- **Apprise multi-backend delivery.** Integrate [Apprise](https://github.com/caronc/apprise) to deliver via Slack, Discord, Telegram, webhooks, SMS, push, etc., in addition to or instead of SMTP — one library, one `apprise.notify(title, body, url)` call, configured per operator. Low effort once the SMTP path exists. See [ideas.md](ideas.md).
...
- **Reminders.** Re-notify approvers about a still-pending request after an interval ("pending since 2 hours ago"). Pairs naturally with the approval-timeout feature. See [ideas.md](ideas.md) and [use-cases/02-shared-account-management.md](use-cases/02-shared-account-management.md).
