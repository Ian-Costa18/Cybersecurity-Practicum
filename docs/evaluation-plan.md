...
**\*Honest caveats on the proxy's column D and beyond.** Column D holds **only under the operator-enforced precondition** that the proxy is the sole holder of the publish token and the sole network path ([constraints.md](constraints.md) §5, §9) — the proxy cannot self-enforce its own placement (threat **PUB-2**: a bypass is now *detected* at bucket ① by out-of-band publish reconciliation (#124), but *prevention* stays operator-③ credential-topology hygiene — detection bounds the exposure window, it does not stop the unmediated credential from publishing). And a colluding quorum of ≥ *m* approvers (threat **CORE-3**, bucket ④) defeats any of these controls by definition. The matrix claims authorization coverage, not omnipotence.
...
**Multi-party approval is being adopted as a primitive by the largest platforms — and every adoption is welded to one platform's own control plane.** AWS shipped a "multi-party approval" capability in 2025, gated through IAM / Organizations and already covering two of its own services (logically air-gapped Backup vaults and Payment Cryptography root-of-trust key imports). Google offers the same primitive twice over: multi-approver grants in Google Cloud Privileged Access Manager, and multi-party approval of sensitive Google Workspace admin settings. Azure has it too, but only as product-specific silos (Multi-User Authorization for Backup vaults, Multi Admin Approval for Intune) — and, tellingly, its *general* privileged-access product (Entra PIM) resolves an activation request on the **first** approver, offering no m-of-n quorum at all. GitHub (deploy environments) and Artifactory (repo promotion) add still more platform-bound instances. Every one is siloed inside a single control plane and gates *that platform's own* operations; **none gates an arbitrary registry publish.** That pattern *reinforces* the gap this project targets rather than filling it: the primitive is proven and its adoption is accelerating, yet a general-purpose, registry-agnostic enforcement point does not exist — the strongest evidence that m-of-n human authorization remains an under-used general primitive. (How the proxy generalizes past package-publishing is future work, discussed in the report, not evaluated here — see Scope.)
...
| **HOST-2** — record / policy tampering | Integrity | Mutate a stored approval record; substitute a signing key; weaken the stored quorum | `Ed25519Verify` fails against the **frozen** snapshot key, or the quorum diverges from live config → the execution-time re-check freezes |
| **IDENT-1** — admin roster forgery | Integrity | A compromised admin quietly enroll-forwards / re-points an approver seat | The admin-action alarm fires (#125): the action lands an attributed, hash-chained audit row and notifies every *other* admin — the takeover cannot be silent |
...
- Multi-party approval as a platform-siloed primitive — AWS Multi-party approval (2025), Google Cloud Privileged Access Manager, Google Workspace multi-party approval, Azure Multi-User Authorization / Intune Multi Admin Approval (and Entra PIM's no-quorum approval).
