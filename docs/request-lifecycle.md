...
- **`approved → frozen`** (#121) when the **execution-time integrity re-check** refuses the request. Before any Post-Approval handoff, `service_types/dispatch.finalize` runs `approvals/integrity.verify_request_integrity`: it verifies every Vote against the signing key **frozen onto the creation snapshot** (ADR 0008; not the live column, so a HOST-2 public-key substitution is caught), and compares the snapshotted `quorum` against the live service config (the policy root of trust an L5 attacker cannot reach). A Vote whose frozen anchor is **null** — an approver unenrolled at creation — is a hard freeze too: the check never falls back to the live, L5-writable key (ADR 0015, HOST-2 null-anchor). On any mismatch the request transitions to `frozen`, emits `request.frozen`, and does **no** handoff — it is parked for manual review rather than published on tampered state ([threat model HOST-2](threat-model/HOST-2-database-write-compromise.md), [ADR 0015](adr/0015-tamper-evident-db-records-two-trust-roots.md)). Two legitimate operator actions also freeze here (the conservative response; ADR 0008 has operators drain pending requests across a policy change): a mid-flight quorum change, and a service **removed** from live config — a decommissioned service's stale request has no policy root of trust left to check against, so it freezes rather than executing unchecked.
...
Each Vote — including a supersession or a withdrawal — is independently signed and emits `request.vote_recorded`. This reframes the old "duplicate decisions are rejected" rule (see [threat model](threat-model/00-overview.md) VOTE-2): an *identical* repeat is still a no-op, but a *changed* decision from the same Approver is an accepted supersession, not a rejected duplicate. Only the authenticated Approver can supersede their own Vote.
...
The catalog is open to additional consumers without changing the lifecycle — for example, **VOTE-4 anomaly detection** (see [threat model](threat-model/00-overview.md)) would subscribe to `request.created` to detect request floods / approval-fatigue bursts.
