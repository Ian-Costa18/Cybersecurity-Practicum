...
bucket: 1
related: [DOS-3, PUB-2, CORE-3, IDENT-5, CORE-2, PUB-3, IDENT-3, CRYPTO-3, VOTE-5]
tests:
  - tests/service_types/one_time/test_compromise_boundary.py::test_m_minus_one_compromised_seats_cannot_publish
  - tests/service_types/one_time/test_compromise_boundary.py::test_compromised_seats_that_only_abstain_leave_it_frozen
  ...
  - tests/approvals/test_votes.py::test_cast_vote_takes_a_row_lock_on_the_request
---
...
| **Capability** | L3 by the ladder — one approver's password **and** TOTP secret together (full account compromise, e.g. malware on their machine or a device compromise). The **L2** route reaches the same seat with only the password, because [IDENT-5](IDENT-5-no-anti-automation-on-authentication-endpoints.md)'s missing rate limit lets a known password be paired with an online TOTP brute-force — the L2→L3 collapse this threat leans on. |
| **What the attacker gains** | One vote: the ability to submit a single approval as the compromised approver. The compromise need not be external — an honest-but-rogue *lone insider* voting their own single seat is the same contained case: one-identity-one-vote holds regardless of whether the seat is stolen or genuinely held, and the m−1 barrier stands either way (coordinated insiders are [CORE-3](CORE-3-insider-collusion.md)). The same position also permits a malicious deny or a withdrawal — the denial direction is [DOS-3](DOS-3-compromised-approver-as-denial-of-service.md)'s threat. |
...
| **Current defenses** | m-of-n quorum, executably demonstrated at the **worst case** by `tests/service_types/one_time/test_compromise_boundary.py` — *t = m−1* fully compromised seats (the harness holds their passwords, TOTP seeds, tokens, and live sessions) cannot publish, whether they vote or merely abstain — at the demo's operating point by `tests/demo/test_act2_compromise.py::test_stolen_seat_and_careless_stamp_freeze_at_two_thirds` (one stolen seat plus one careless rubber-stamp freezes at 2/3), and at the threshold by `test_quorum_reached_only_at_the_threshold`, `test_two_approvals_over_http_reach_quorum`, `test_a_non_eligible_user_cannot_vote` (`tests/approvals/test_votes.py`, `test_approve.py`), with `tests/test_execution.py::test_a_denied_request_never_reaches_pypi` as the black-box oracle — below quorum, the PyPI mock is never invoked. Concurrent votes cannot race the count past the threshold: vote application takes a row lock on the request (`tests/approvals/test_votes.py::test_cast_vote_takes_a_row_lock_on_the_request`, `SELECT … FOR UPDATE`). Password + TOTP two-factor authentication — but see [IDENT-5](IDENT-5-no-anti-automation-on-authentication-endpoints.md): absent rate limiting, the TOTP factor can be brute-forced online once the password is known, so this factor is weaker than it appears until anti-automation lands. Ed25519 signing: approvals are cryptographically bound to the authenticated identity and cannot be transferred or replayed against a different request. |
...
it, the PyPI mock is never invoked (`test_a_denied_request_never_reaches_pypi` is the
literal oracle — the publish path is never reached without m signed votes).
