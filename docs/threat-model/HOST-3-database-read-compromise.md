...
tokens) or *wrapped under a key the reader does not hold* (the private key, under the
password-derived key). The TOTP secret is the one credential that is neither. It need not
be: the proxy only checks TOTP at interactive login, and login always presents the
password, so the secret can be wrapped under the same password-derived key as the private
key ([#122](https://github.com/Ian-Costa18/Cybersecurity-Practicum/issues/122)) rather than
stored raw. Until then it is the sole plaintext credential, and it is what makes a bare read
escalate toward full account takeover.
