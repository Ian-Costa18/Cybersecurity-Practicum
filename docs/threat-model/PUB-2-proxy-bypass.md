...
commodity theft of a credential known to exist. It is not `low`, because nothing verifies
exclusivity and nothing detects a violation until
[#124](https://github.com/Ian-Costa18/Cybersecurity-Practicum/issues/124) lands — a missed
credential stays live indefinitely. Severity residual `critical`: an unauthorized artifact
reaches PyPI directly and unobserved — the top rung of the mission ladder.
...
- **Out-of-band publish reconciliation (detect + alert)** — [#124](https://github.com/Ian-Costa18/Cybersecurity-Practicum/issues/124) — promotes ③ → ① (detection tier): reconciling PyPI releases against the proxy's publish log makes an exclusivity violation observable, with the alert as the executable oracle.
