...
| **What they cannot do** | Turn one capture into more than one vote — each accepted TOTP is burned per `(user, time-step)` ([VOTE-2](VOTE-2-captured-credential-replay.md), tested); the vote dies with the request's terminal state. Escape notice structurally: the phished vote is signed, attributed to the victim, and visible — [INFO-1](INFO-1-information-disclosure-via-quorum-status-approver-visibility.md)'s endorser visibility is what lets the real Approver spot a vote they never cast, and under the append-only vote model ([ADR 0009](../adr/0009-append-only-vote-model.md)) they can **supersede** it while the request is still pending. Publish alone — one phished Approver is one seat of m; a publish requires phishing m Approvers inside one pending window. |
...
family: [IDENT-3](IDENT-3-notification-channel-interception.md) owns the channel the lure rides,
IDENT-4 owns the capture, [VOTE-2](VOTE-2-captured-credential-replay.md) owns what captured material
is worth, and [VOTE-3](VOTE-3-browser-borne-approval-coercion.md) owns the sibling coercion that
...
tests enumerated in [VOTE-2](VOTE-2-captured-credential-replay.md)), an open recovery path
(supersession while pending, surfaced by [INFO-1](INFO-1-information-disclosure-via-quorum-status-approver-visibility.md)'s
...
[VOTE-3](VOTE-3-browser-borne-approval-coercion.md) nothing architecturally forecloses the
...
[VOTE-2](VOTE-2-captured-credential-replay.md), which owns the replay of what IDENT-4 captures.

## Planned defenses

- **Phishing-resistant per-vote authentication (WebAuthn/FIDO2 passkeys)** — [#129](https://github.com/Ian-Costa18/Cybersecurity-Practicum/issues/129) — the only mechanism that *closes* the invariant rather than shrinking it: an origin-bound credential will not produce an assertion for the wrong origin, no matter how convincing the page, defeating both credential capture and live AiTM relays. Non-trivial: the Ed25519 signing key is wrapped under the password today; the issue records three shapes (passkey as second factor with the password retained; PRF-extension key wrapping; layered). **Descoped to future work (grill 2026-07-04):** building WebAuthn was judged too large for the practicum timeline — it is the only bucket-① promotion that would require a whole new auth protocol rather than reuse of existing primitives — so IDENT-4 stays bucket ② (argued by design) with residual likelihood **high** as the accepted position. If ever built, Shape 1 (passkey as second factor, password retained) is the target and would promote the capture-prevention leg toward ①.
