...
| **Operator configuration** | Deploy exclusively over HTTPS with a valid CA-issued certificate and enable HSTS (both already on the [operator checklist](operator-checklist.md)). Monitor Certificate Transparency logs for unexpected issuance against the proxy's domain. Pin the proxy's URL in approver onboarding (a bookmark defeats DNS-luring). Keep the origin's DNS registrar account under 2FA — it is part of this trust chain. |
