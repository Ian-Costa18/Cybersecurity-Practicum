...
# HOST-4 — Database Repudiation Attack

| | |
|---|---|
| **Category** | Repudiation |
| **Capability** | L5 — write/delete access to the database. The content-tampering sibling at the same rung is [HOST-2](HOST-2-database-write-compromise.md) (rewrite a record); the availability twin is [DOS-2](DOS-2-destructive-availability-attack.md) (destroy the machinery). |
...
| **What they cannot do** | Alter a *retained* record's content undetectably — that is [HOST-2](HOST-2-database-write-compromise.md)'s territory (Ed25519 catches content edits). Publish, approve, or forge through this threat — it erases the past, it does not manufacture a future action. Break the system's ability to operate — deleting operational data to *halt* publishing is availability loss ([DOS-2](DOS-2-destructive-availability-attack.md)), not repudiation. |
...
| **Operator configuration** | Give the proxy's DB role INSERT on the approval/audit tables but **not** DELETE or UPDATE. Mirror the audit trail to an external append-only store (WORM object storage, an append-only log service) out of reach of the database capability rungs — the defense family shared with [DOS-2](DOS-2-destructive-availability-attack.md). Keep offsite backups so a deletion can at least be *reconstructed* after the fact. Alert on any DELETE against the records tables. |

**Boundaries.** vs. [HOST-2](HOST-2-database-write-compromise.md): HOST-2 owns *you cannot rewrite a Vote's content undetectably* (Ed25519 catches content edits); HOST-4 owns *you can delete the whole Vote and nothing notices* (per-record signatures do not detect removal — the trail has no hash chain). Same capability rung, opposite operations. vs. [DOS-2](DOS-2-destructive-availability-attack.md): deleting records to *break the system's ability to function* is availability loss (DOS-2); deleting records to *hide that an action happened* is repudiation (here). If one wipe does both, the availability consequence is rated under DOS-2 and the accountability consequence here. vs. [IDENT-1](IDENT-1-admin-account-compromise.md): the quiet enroll-forward takeover leaves only unsigned `AuditLog` rows as its trace — HOST-4 is the act of erasing exactly those rows to complete the cover-up.

The mapping is one technique. **T1070 (Indicator Removal):** the adversary deletes or manipulates artifacts — here approval and audit records — to remove evidence of their activity, which is the whole of what this threat narrates. T1562.001 (*Impair Defenses: Disable or Modify Tools*) was considered and dropped: disabling the audit-*writing* path means modifying proxy code, which is beyond a database-write capability (L5) — that position is [HOST-1](HOST-1-proxy-host-compromise.md)'s, not this threat's.
...
- **Tamper-evident `AuditLog` (hash chain or external append-only sink, plus acting-actor identity on each row)** — #121 — makes whole-record deletion and reordering detectable rather than silent (**③ → ②**). The same issue's signed-snapshot leg serves [HOST-2](HOST-2-database-write-compromise.md).
