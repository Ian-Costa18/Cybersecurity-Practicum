...
| **Operator configuration** | Until limits land, this is the whole defense: monitor approval-request and upload volume and investigate bursts; deactivate requester accounts (or revoke tokens) behaving anomalously; watch database/storage growth, since staged artifacts live in the DB; and cap concurrent connections (and idle-timeout long-lived ones) per client at the reverse proxy to bound the single-worker stream-starvation leg. |
