...
- [ ] Expose the proxy only to the audiences that actually need it — prefer an internal network or VPN over an internet-facing origin. Where requesters and approvers are internal, the proxy need not be publicly reachable, and keeping it off the public internet means a request-token harvested from a developer machine (CORE-2) cannot even be *used* without network position, and shrinks the remotely reachable attack surface (CODE-1). This is the network-reach half of the PUB-2 posture in which the sole upload credential lives inside the proxy: a stolen proxy token opens only a deny-able request, and reaching the endpoint to open it is itself gated.
