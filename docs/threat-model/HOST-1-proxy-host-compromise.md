...
bucket: 4
related: [HOST-3, HOST-2, PUB-1, CODE-2, CODE-1, DOS-2, IDENT-6, HOST-5, VOTE-5]
tests:
...
CORE-2's improved story, counted there.)

**Why bucket ④.** The discriminator this catalog uses for accepted limitations, shared with
[CODE-2](CODE-2-supply-chain-attack-on-the-proxy-itself.md): *if the operator cannot completely
...
unprotected *at rest*, which is [HOST-3](HOST-3-database-read-compromise.md)'s surface, not the
