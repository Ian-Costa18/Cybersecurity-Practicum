...
| **Current defenses** | AES-256-GCM encryption of private keys at rest: the plaintext private key is never stored. bcrypt password hashing at cost ≥ 12: makes offline cracking expensive; unique per-user salts prevent precomputed tables. 128-bit random salt per user (PBKDF2): prevents cross-user rainbow tables. |
...
| **Category** | Elevation of Privilege |
| **Capability** | L1, L2 |
| **What the attacker gains** | If an approval link can be reused, an attacker who obtains a previously used link could attempt to authenticate with a compromised approver account and submit a second approval on an already-decided request. |
| **What they cannot do** | Submit a duplicate decision from the same approver for the same request — the proxy rejects duplicate `(approver_id, approval_request_id)` pairs. Reuse a link from one request to approve a different request — links are scoped to a specific `approval_id`. |
| **Current defenses** | Approval records carry `approval_request_id`; duplicate decisions from the same approver for the same request are rejected at the application layer. Authentication is required on every link visit; obtaining the link alone is insufficient. |
| **Planned defenses** | Approval link expiration (currently absent; links do not expire): adds a time-based barrier for links that were captured but not used. |
...
| **What they cannot do** | Succeed in this attack — hash binding blocks it. The payload is hashed at upload time; approvers approve that specific hash; if the artifact changes, the stored hash will not match, and publication is blocked. |
| **Current defenses** | Hash binding: payload SHA-256 is computed immediately on upload and recorded in the approval request. Any substitution causes a hash mismatch and publication is blocked regardless of who is running the proxy. |
| **Planned defenses** | The hash binding mechanism is a fundamental design principle and already fully effective. Future enhancements: surface the hash prominently in the approve/deny UI so approvers can cross-check against their local copy. |
...
| T8 | Approval link replay | L1/L2 | Yes — duplicate decisions rejected | Use TLS for link distribution |
