...
5. **[Publishing]** As an Approver, I want the approval link to remain reachable in the portal even if email delivery fails, so that a flaky mail server never strands a pending request (portal fallback).
...
- **Notification is best-effort for *delivery* but guaranteed for *solicitation and reachability*.** Every Approval Request emits a notification to every eligible Approver; if SMTP fails, the link is reachable via portal fallback. MVP is **email-only**; multi-channel (Apprise) is future work.
- **The proxy is assumed to be the sole holder of publish credentials.** For PyPI this means a project owned by a service account with no human maintainers, with the proxy holding the only upload token. Enforcing the proxy as the sole network/credential path is an **operational precondition**, not something the proxy self-enforces ([constraints.md](constraints.md) §5, §7). The long-term fix is native registry support; this project advocates for it.
...
4. **Usability — functional completeness + automated solicitation.** Happy-path integration tests green end-to-end for *both* use cases (submit → notify → inspect → approve to quorum → publish/forward → outcome), **plus** a notification emitted per eligible Approver, **plus** the fallback link reachable when SMTP is down. Friction (deliberate re-auth-per-vote, wait-for-quorum) is reported qualitatively, not faked as a satisfaction score.
...
- **Clock/scheduler, approval-link expiry, multi-channel notification, threshold signatures (MuSig2/FROST).** All future work ([ideas.md](ideas.md), [mvp.md](mvp.md)).
...
- **Transparency log.** Peer feedback (Mikhail, Thea / CHAINIAC) suggested a Sigstore-style transparency log. The MVP's signed, offline-verifiable audit trail is the first step; an external write-once log is future hardening ([threat-model.md](threat-model.md) T6).
